...
2. restore／publishに同じ`win-x64`、`SelfContained=true`、`PublishSingleFile=true`、`IncludeNativeLibrariesForSelfExtract=true`、`IncludeAllContentForSelfExtract=true`を渡す。trimming、ReadyToRun、圧縮は無効、symbolsは非配布とする。S01で確認したbuild-only依存`Microsoft.NET.ILLink.Tasks 10.0.11`だけをApp側でlockし、Coreの追加lockやproduction projectを増やさず、analyzer／integrity検証を無効化しない。
