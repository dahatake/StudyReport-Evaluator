...
| **並列度** | 1〜8、既定4。変更は次回run用。run中にrate limitを検出した場合は、この値を上限に有効並列度を一時的に下げます |
...
- 参照回答・通常評価・固有評価はrunで選んだ同じmodelとreasoning effortを使います。Similarityはlocal deterministic計算のためLLM modelを使いません。固定`auto`の別検証はありません。
