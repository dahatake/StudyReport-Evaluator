...
通常モデル、並列度（1〜16）、指定出力先の利用者編集は**設定 → 共通**で行います。実行画面には次回の実効値と、今回／前回runの開始時に固定した値を分けて表示します。Reference／Normal／Specialはrun開始時の通常モデルと同じreasoning effort（既定希望`low`、非対応時は未指定）を使います。Similarityはローカル計算です。
