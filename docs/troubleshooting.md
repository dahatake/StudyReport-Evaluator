...
`nano-AI units`とpremium request消費量はSDK報告値です。アプリはAIクレジット・円・ドル・アカウント残量へ換算しません。入力／出力tokenと重複し得る推論／cache内訳を合計tokenとして加算しないでください。記録対象、保存先、保持期間は[データとprivacy](privacy-and-data-handling.md#ジョブコストログ)を確認してください。
...
| 「保存失敗: 設定が不正」（`InvalidSettings`） | 並列度1〜8、絶対出力path、採点定義・Prompt・配点の検証を確認し、現在のdraftを修正する。旧fileは変更しない |
