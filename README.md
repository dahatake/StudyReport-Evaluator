...
2. **変更 → 共通**で、列挙された通常モデル、並列度（1〜8、既定4）、指定出力先を確認・変更します。保存希望modelが利用不可なら未選択のままで、別modelへfallbackしません。Reference／通常評価／固有評価は同じ通常モデルとrun-level reasoning effort（既定希望`low`、非対応時は未指定）を使います。Similarityはローカル計算でLLMを呼びません。
