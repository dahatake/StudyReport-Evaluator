...
- 要求定義書（v4.6）の§7.6を改訂し、1回の試行（attempt）のtimeoutの既定を120秒から、Copilot SDKの`SendAndWaitAsync`の既定と同じ60秒へ変更。60秒には起動・認証確認・session作成も含まれるため、AIの応答待ちに使える時間はその分短くなります。
- 通常評価・参照回答生成・固有評価・類似度評価の全operationで、reasoning effortを`medium`に指定するように変更（答案の定量化にhigh以上は使いません）。指定するのは、SDKのモデル一覧がreasoning effort対応と示し、`medium`を列挙したモデルだけです。`auto`と非対応モデル（例：claude-haiku-4.5）は指定するとsession作成が失敗するため指定せず、ランタイムの既定に任せます。これを理由に実行を拒否したり、別モデルへ切り替えたりはしません。
